...
TEXT_SESSION=/tmp/ralph_direct_pi_text.session
IMAGE_SESSION=/tmp/ralph_direct_pi_image.session
CURRENT_PANE_ID=

log() {
  printf '[verify:direct-pi] %s\n' "$1"
}
...
seed_text_clipboard() {
  "$POWERSHELL_EXE" -NoProfile -ExecutionPolicy Bypass -Command 'Set-Clipboard -Value "direct-pi-text-proof"'
}
...
match = re.search(r'>\s*(direct-pi-text-proof)', text)
if not match:
    raise SystemExit(1)
print(match.group(1))
PY
  ...
  if [[ "$actual" != 'direct-pi-text-proof' ]]; then
...
verify_image_case() {
  seed_image_clipboard
  start_pi_session "$IMAGE_SESSION"
  send_text $'\e[23~'
  sleep 5
  stop_pi_session
...
print(f"[verify:direct-pi] image proof: {path} size={image.size} mode={image.mode}")
PY
}

verify_text_case
verify_image_case
log 'direct PI smart paste verified'
